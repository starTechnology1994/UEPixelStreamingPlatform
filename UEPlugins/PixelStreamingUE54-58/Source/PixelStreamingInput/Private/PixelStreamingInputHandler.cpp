...
namespace UE::PixelStreamingInput
{
	...
	void FPixelStreamingInputHandler::HandleOnKeyDown(FMemoryReader Ar)
	{
		...
		// 键盘事件来自浏览器，必须确保像素流 wrapper 是当前平台应用，否则
		// FSlateApplication::OnKeyDown 会从服务器本地（无桌面=空）读取修饰键状态，导致 Ctrl+A 等组合键失效。
		// 鼠标移出画面（MouseLeave）会把平台应用恢复为服务器本地应用，这里在按键时强制切回。
		if (PixelStreamerApplicationWrapper.IsValid() &&
			FSlateApplication::Get().GetPlatformApplication().Get() != PixelStreamerApplicationWrapper.Get())
		{
			FSlateApplication::Get().OverridePlatformApplication(PixelStreamerApplicationWrapper);
		}

		// 更新浏览器报告的修饰键状态（Ctrl/Shift/Alt），解决无桌面服务器上 GetModifierKeys() 永远为空的问题
		...
		if (FilterKey(*AgnosticKey))
		{
			...
			UE_LOG(LogPixelStreamingInputHandler, Verbose, TEXT("KEY_DOWN: Key = %d; Character = %d; IsRepeat = %ls"), Key, Character, bIsRepeat ? TEXT("True") : TEXT("False"));
	...
	void FPixelStreamingInputHandler::HandleOnKeyUp(FMemoryReader Ar)
	{
		...
		// 与 KeyDown 对称：确保像素流 wrapper 是当前平台应用
		if (PixelStreamerApplicationWrapper.IsValid() &&
			FSlateApplication::Get().GetPlatformApplication().Get() != PixelStreamerApplicationWrapper.Get())
		{
			FSlateApplication::Get().OverridePlatformApplication(PixelStreamerApplicationWrapper);
		...
		// 清除浏览器报告的修饰键状态（与 KeyDown 对称），避免 Ctrl/Shift/Alt 松开后永远保持按下。
		// 必须在 FilterKey 之前执行，保证修饰键状态始终同步。
		if (PixelStreamerApplicationWrapper.IsValid())
		{
	...
	void FPixelStreamingInputHandler::HandleOnControllerButtonPressed(FMemoryReader Ar)
	{
		...
		UE_LOG(LogPixelStreamingInputHandler, Verbose, TEXT("GAMEPAD_PRESSED: ControllerId = %d; KeyName = %ls; IsRepeat = %ls;"), ControllerId.GetId(), *ButtonPtr->ToString(), bIsRepeat ? TEXT("True") : TEXT("False"));
	}
	...
	void FPixelStreamingInputHandler::HandleOnControllerButtonReleased(FMemoryReader Ar)
	{
		...
		UE_LOG(LogPixelStreamingInputHandler, Verbose, TEXT("GAMEPAD_RELEASED: ControllerId = %d; KeyName = %ls;"), ControllerId.GetId(), *ButtonPtr->ToString());
	}
	...
	void FPixelStreamingInputHandler::HandleOnXRButtonTouched(FMemoryReader Ar)
	{
		...
		UE_LOG(LogPixelStreamingInputHandler, Verbose, TEXT("XR_TOUCHED: ControllerId = %d; KeyName = %ls; IsRepeat = %ls;"), ControllerId.GetId(), *ButtonPtr->ToString(), bIsRepeat ? TEXT("True") : TEXT("False"));
	}
	...
	void FPixelStreamingInputHandler::HandleOnXRButtonTouchReleased(FMemoryReader Ar)
	{
		...
		UE_LOG(LogPixelStreamingInputHandler, Verbose, TEXT("XR_TOUCH_RELEASED: ControllerId = %d; KeyName = %ls; IsRepeat = %ls;"), ControllerId.GetId(), *ButtonPtr->ToString(), bIsRepeat ? TEXT("True") : TEXT("False"));
	}
	...
	void FPixelStreamingInputHandler::HandleOnXRButtonPressed(FMemoryReader Ar)
	{
		...
		{
			MessageHandler->OnControllerButtonPressed(ButtonPtr->GetFName(), DeviceMapper.GetPrimaryPlatformUser(), ControllerId, bIsRepeat);
			UE_LOG(LogPixelStreamingInputHandler, Verbose, TEXT("XR_PRESSED: ControllerId = %d; KeyName = %ls; IsRepeat = %ls"), ControllerId.GetId(), *ButtonPtr->ToString(), bIsRepeat ? TEXT("True") : TEXT("False"));
		}
		...
		if(AxisPtr)
		{
			...
			UE_LOG(LogPixelStreamingInputHandler, Verbose, TEXT("XR_ANALOG: ControllerId = %d; KeyName = %ls; IsRepeat = False; AnalogValue = %.4f; [Queued for Tick()]"), ControllerId.GetId(), *AxisPtr->ToString(), AnalogValue.Value);
	...
	void FPixelStreamingInputHandler::HandleOnXRButtonReleased(FMemoryReader Ar)
	{
		...
		{
			MessageHandler->OnControllerButtonReleased(ButtonPtr->GetFName(), DeviceMapper.GetPrimaryPlatformUser(), ControllerId, bIsRepeat);
			UE_LOG(LogPixelStreamingInputHandler, Verbose, TEXT("XR_RELEASED: ControllerId = %d; KeyName = %ls; IsRepeat = %ls;"), ControllerId.GetId(), *ButtonPtr->ToString(), bIsRepeat ? TEXT("True") : TEXT("False"));
		}
	}
	...
	void FPixelStreamingInputHandler::HandleOnXRAnalog(FMemoryReader Ar)
	{
		...
		UE_LOG(LogPixelStreamingInputHandler, Verbose, TEXT("XR_ANALOG: ControllerId = %d; KeyName = %ls; IsRepeat = False; AnalogValue = %.4f; [Queued for Tick()]"), ControllerId.GetId(), *AnalogKeyPtr->ToString(), AnalogValue.Value);
		AnalogEventsReceivedThisTick.FindOrAdd(ControllerId).FindOrAdd(AnalogKeyPtr) = AnalogValue;
	}
	...
	void FPixelStreamingInputHandler::HandleOnCommand(FString SourceId, FMemoryReader Ar)
	{
		...
		UE_LOG(LogPixelStreamingInputHandler, Verbose, TEXT("Command: %ls"), *Descriptor);
	...
	void FPixelStreamingInputHandler::ProcessLatestAnalogInputFromThisTick()
	{
		for (auto AnalogInputIt = AnalogEventsReceivedThisTick.CreateIterator(); AnalogInputIt; ++AnalogInputIt)
		{
			for (auto FKeyIt = AnalogInputIt->Value.CreateIterator(); FKeyIt; ++FKeyIt)
			{
				...
				UE_LOG(LogPixelStreamingInputHandler, Verbose, TEXT("TICKED ANALOG Input: ControllerId = %d; KeyName = %ls; IsRepeat = %ls; AnalogValue = %.4f; Handled = %ls; [Queued for Tick()]"), ControllerId.GetId(), *Key->ToString(), bIsRepeat ? TEXT("True") : TEXT("False"), AnalogValue.Value, bHandled ? TEXT("True") : TEXT("False"));
