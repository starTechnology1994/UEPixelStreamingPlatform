...
namespace UE::PixelStreamingServers
{
	...
	bool FMonitoredServerBase::FindServerAbsPath(FLaunchArgs& InLaunchArgs, FString& OutServerAbsPath)
	{
		...
		if(!bUseServerBinary)
		{
			...
			UE_LOG(LogPixelStreamingServers, Log, TEXT("Downloaded server not found at - %ls"), *OutServerAbsPath);
		}
		else
		{
			...
			UE_LOG(LogPixelStreamingServers, Log, TEXT("Server binary not found at - %ls"), *OutServerAbsPath);
		}
		return false;
	}
