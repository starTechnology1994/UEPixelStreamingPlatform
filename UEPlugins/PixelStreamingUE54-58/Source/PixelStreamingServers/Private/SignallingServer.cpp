...
namespace UE::PixelStreamingServers
{
	...
	TSharedPtr<FJsonObject> FSignallingServer::ParseMessage(const FString& InMessage, FString& OutMessageType) const
	{
		...
		if (!JSONObj)
		{
			UE_LOG(LogPixelStreamingServers, Error, TEXT("Failed to parse message: %ls"), *InMessage);
			return nullptr;
		}

		if (!JSONObj->TryGetStringField(TEXT("type"), OutMessageType))
		{
			UE_LOG(LogPixelStreamingServers, Warning, TEXT("Incoming message did not contain a 'type' field: %ls"), *InMessage);
			return nullptr;
		}
	...
	{
		UE_LOG(LogPixelStreamingServers, Log, TEXT("Subscribing player %d to streamer %ls"), PlayerConnectionId, *StreamerName);

		uint16 StreamerConnectionId;
		if (!StreamersWS->GetNamedConnection(StreamerName, StreamerConnectionId))
		{
			UE_LOG(LogPixelStreamingServers, Log, TEXT("Streamer name %ls does not exist"), *StreamerName);
			return;
		}
	...
	void FSignallingServer::SendPlayerMessage(uint16 PlayerId, TSharedPtr<FJsonObject> JSONObj)
	{
		...
		UE_LOG(LogPixelStreamingServers, Log, TEXT("Sending to player id=%d: %ls"), PlayerId, *MessageString);
	...
	{
		const FString MessageString = Utils::ToString(JSONObj);
		UE_LOG(LogPixelStreamingServers, Log, TEXT("Sending to streamer id=%d: %ls"), StreamerId, *MessageString);
	...
	void FSignallingServer::OnStreamerMessage(uint16 ConnectionId, TArrayView<uint8> Message)
	{
		...
		UE_LOG(LogPixelStreamingServers, Log, TEXT("From Streamer id=%d: %ls"), ConnectionId, *Msg);
		...
		else
		{
			...
			{
				UE_LOG(LogPixelStreamingServers, Warning, TEXT("Message did not contain a field called 'playerId' - message=%ls"), *Msg);
				return;
			}
	...
	{
		const FString Msg = Utils::ToString(Message);
		UE_LOG(LogPixelStreamingServers, Log, TEXT("From Player id=%d: %ls"), ConnectionId, *Msg);
		...
		else
		{
			if (!PlayerSubscriptions.Contains(ConnectionId))
			{
				...
				UE_LOG(LogPixelStreamingServers, Log, TEXT("Player %d attempted to send an outgoing message without having subscribed first. Defaulting to %ls"), ConnectionId, *StreamerConnections[0]);
