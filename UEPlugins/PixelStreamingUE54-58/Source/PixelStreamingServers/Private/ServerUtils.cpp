...
namespace UE::PixelStreamingServers::Utils
{
	...
	TSharedPtr<FMonitoredProcess> LaunchChildProcess(FString ExecutableAbsPath, FString Args, FString LogPrefix, bool bRunAsScript)
	{
		...
		{
			UE_LOG(LogPixelStreamingServers, Error, TEXT("Cannot start child process - the specified file did not exist. File=%ls"), *ExecutableAbsPath);
...
// Get the executable we will use to run the scripts (e.g. cmd.exe on Windows)
		...
		ChildProcess->OnOutput().BindLambda([LogPrefix](FString Output) {
			UE_LOG(LogPixelStreamingServers, Log, TEXT("%ls - %ls"), *LogPrefix, *Output);
		});
		// Run the child process
		UE_LOG(LogPixelStreamingServers, Log, TEXT("Launch child process - %ls %ls"), *ExecutableAbsPath, *Args);
		ChildProcess->Launch();
		return ChildProcess;
	}
	...
	bool PopulateCirrusEndPoints(FLaunchArgs& InLaunchArgs, TMap<EEndpoint, FURL>& OutEndPoints)
	{
		...
		int FallbackStreamerPort = UE::PixelStreaming::GetNextAvailablePort();
		int FallbackSFUPort = UE::PixelStreaming::GetNextAvailablePort();
		int FallbackHttpPort = UE::PixelStreaming::GetNextAvailablePort();
		int FallbackMatchmakerPort = UE::PixelStreaming::GetNextAvailablePort();
		if (FallbackStreamerPort == -1)
		{
			UE_LOG(LogPixelStreamingServers, Warning, TEXT("Failed to find an available port for streamer connections"));
			return false;
		}
		if (FallbackSFUPort == -1)
		{
			UE_LOG(LogPixelStreamingServers, Warning, TEXT("Failed to find an available port for SFU connections"));
			return false;
		}
		if (FallbackHttpPort == -1)
		{
			UE_LOG(LogPixelStreamingServers, Warning, TEXT("Failed to find an available port for http connections"));
			return false;
		}
		if (FallbackMatchmakerPort == -1)
		{
			UE_LOG(LogPixelStreamingServers, Warning, TEXT("Failed to find an available port for matchmaker connections"));
			return false;
		}
		...
		FString MatchmakerPort = Utils::QueryOrSetProcessArgs(InLaunchArgs, TEXT("--MatchmakerPort="), FString::FromInt(FallbackMatchmakerPort));
