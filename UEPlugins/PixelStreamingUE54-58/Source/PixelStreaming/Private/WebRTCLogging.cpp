...
private:
		...
		const char* tag) override
	{
		...
		switch (severity)
		{
			case rtc::LS_VERBOSE:
			{
				UE_LOG(LogPixelStreamingWebRTC, Verbose, TEXT("%ls"), *Msg);
				break;
			}
			case rtc::LS_INFO:
			{
				UE_LOG(LogPixelStreamingWebRTC, Log, TEXT("%ls"), *Msg);
				break;
			}
			case rtc::LS_WARNING:
			{
				UE_LOG(LogPixelStreamingWebRTC, Warning, TEXT("%ls"), *Msg);
				break;
			}
			case rtc::LS_ERROR:
			{
				UE_LOG(LogPixelStreamingWebRTC, Error, TEXT("%ls"), *Msg);
				break;
			}
		}
#endif
	...
	{
		//unimplemented();
		UE_LOG(LogPixelStreamingWebRTC, Verbose, TEXT("%ls"), *FString(message.c_str()));
	}
};
