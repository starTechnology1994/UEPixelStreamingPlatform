...
namespace UE::PixelStreaming
{
	...
	{
		UE_LOG(LogPixelStreaming, Log, TEXT("SendDataChannelMessageToStreamer: %d, %ls"), Id, *Body);
	...
	bool FWaitForFrameReceived::Update()
	{
		if (VideoSink && VideoSink->HasReceivedFrame())
		{
			...
			{
				UE_LOG(LogPixelStreaming, Error, TEXT("%ls"), *WidthTestString);
			}
			else
			{
				UE_LOG(LogPixelStreaming, Log, TEXT("%ls"), *WidthTestString);
			}
			...
			{
				UE_LOG(LogPixelStreaming, Error, TEXT("%ls"), *PixelTestString);
			}
			else
			{
				UE_LOG(LogPixelStreaming, Log, TEXT("%ls"), *PixelTestString);
			}
	...
	TSharedPtr<UE::PixelStreamingServers::IServer> CreateSignallingServer(int StreamerPort, int PlayerPort, bool bCreateLegacySignallingServer)
	{
		...
		UE_LOG(LogPixelStreaming, Log, TEXT("Signalling server launched=%ls"), bLaunchedSignallingServer ? TEXT("true") : TEXT("false"));
