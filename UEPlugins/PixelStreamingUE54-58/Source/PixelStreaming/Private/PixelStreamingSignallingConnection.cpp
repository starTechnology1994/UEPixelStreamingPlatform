...
void FPixelStreamingSignallingConnection::Connect(FString InUrl, bool bIsReconnect)
{
	...
	if (bIsReconnect)
	{
		UE_LOG(LogPixelStreamingSS, Log, TEXT("Reconnecting to SS %ls"), *Url);
	}
	else
	{
		UE_LOG(LogPixelStreamingSS, Log, TEXT("Connecting to SS %ls"), *Url);
	}
...
void FPixelStreamingSignallingConnection::Disconnect(FString Reason)
{
	...
	UE_LOG(LogPixelStreamingSS, Log, TEXT("Closing websocket to SS %ls"), *Url);
...
void FPixelStreamingSignallingConnection::SendOffer(FPixelStreamingPlayerId PlayerId, const webrtc::SessionDescriptionInterface& SDP)
{
	...
	UE_LOG(LogPixelStreamingSS, Log, TEXT("Sending player=%ls \"offer\" to SS %ls"), *PlayerId, *Url);
...
void FPixelStreamingSignallingConnection::SendAnswer(FPixelStreamingPlayerId PlayerId, const webrtc::SessionDescriptionInterface& SDP)
{
	...
	if (SDP.ToString(&SdpAnsi))
	{
		...
		UE_LOG(LogPixelStreamingSS, Log, TEXT("Sending player=%ls \"answer\" to SS %ls"), *PlayerId, *Url);
...
void FPixelStreamingSignallingConnection::SendDisconnectPlayer(FPixelStreamingPlayerId PlayerId, const FString& Reason)
{
	...
	UE_LOG(LogPixelStreamingSS, Verbose, TEXT("Sending player=%ls \"disconnectPlayer\" to SS %ls"), *PlayerId, *Url);
...
{
	UE_LOG(LogPixelStreamingSS, Log, TEXT("Connected to SS %ls"), *Url);
...
{
	UE_LOG(LogPixelStreamingSS, Log, TEXT("Failed to connect to SS %ls - signalling server may not be up yet. Message: \"%ls\""), *Url, *Error);
...
{
	UE_LOG(LogPixelStreamingSS, Log, TEXT("Closed connection to SS %ls - \n\tstatus %d\n\treason: %ls\n\twas clean: %ls"), *Url, StatusCode, *Reason, bWasClean ? TEXT("true") : TEXT("false"));
...
void FPixelStreamingSignallingConnection::OnMessage(const FString& Msg)
{
	...
	{
		UE_LOG(LogPixelStreamingSS, Error, TEXT("Unsupported message `%ls` received from SS"), *MsgType);
	}
}
...
void FPixelStreamingSignallingConnection::OnPlayerConnected(const FJsonObjectPtr& Json)
{
	...
	UE_LOG(LogPixelStreamingSS, Log, TEXT("Got player connected, player id=%ls"), *PlayerId);
...
{
	UE_LOG(LogPixelStreamingSS, Error, TEXT("player %ls: %ls"), *PlayerId, *Msg);
