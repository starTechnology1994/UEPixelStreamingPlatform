...
const FPixelStreamingPlayerId INVALID_PLAYER_ID = ToPlayerId(FString(TEXT("Invalid Player Id")));
const FPixelStreamingPlayerId SFU_PLAYER_ID = FString(TEXT("1"));
...
namespace UE::PixelStreaming
{
	...
	void FPixelStreamingModule::StartupModule()
	{
		...
		if (!(RHIType == ERHIInterfaceType::D3D11 || RHIType == ERHIInterfaceType::D3D12 || RHIType == ERHIInterfaceType::Vulkan || RHIType == ERHIInterfaceType::Metal))
		{
			...
			UE_LOG(LogPixelStreaming, Warning, TEXT("Only D3D11/D3D12/Vulkan/Metal Dynamic RHI is supported. Detected %ls"), GDynamicRHI != nullptr ? GDynamicRHI->GetName() : TEXT("[null]"));
#endif
			return;
		}
	...
	TSharedPtr<IPixelStreamingStreamer> FPixelStreamingModule::CreateStreamer(const FString& StreamerId)
	{
		...
		if (!Settings::GetSignallingServerUrl(SignallingServerURL))
		{
			...
			if (Settings::GetSignallingServerIP(SignallingServerIP) && Settings::GetSignallingServerPort(SignallingServerPort))
			{
				...
				UE_LOG(LogPixelStreaming, Warning, TEXT("PixelStreamingIP and PixelStreamingPort are deprecated flags. Use PixelStreamingURL instead. eg. -PixelStreamingURL=ws://%ls:%d"), *SignallingServerIP, SignallingServerPort);
	...
	{
		UE_LOG(LogPixelStreaming, Log, TEXT("PixelStreaming streamer ID: %ls"), *Settings::GetDefaultStreamerID());
		...
		if (!GIsEditor)
		{
			...
			if (UGameEngine* GameEngine = Cast<UGameEngine>(GEngine); GameEngine != nullptr)
			{
				TSharedPtr<SWindow> TargetWindow = GameEngine->GameViewport ? GameEngine->GameViewport->GetWindow() : nullptr;
				if (!TargetWindow.IsValid())
				{
					TargetWindow = GameEngine->GameViewportWindow.Pin();
				}
			...
			{
				UE_LOG(LogPixelStreaming, Error, TEXT("Cannot set target window - GEngine is not valid."));
			}
		}
	...
	bool FPixelStreamingModule::IsPlatformCompatible() const
	{
		...
		if (!bWin8OrHigher)
		{
			...
			UE_LOG(LogPixelStreaming, Error, TEXT("%ls"), *ErrorString);
			bCompatible = false;
		}
#endif
	...
	void FPixelStreamingModule::HandleUIInteraction(FMemoryReader Ar)
	{
		...
		UE_LOG(LogPixelStreaming, Verbose, TEXT("UIInteraction: %ls"), *Descriptor);
