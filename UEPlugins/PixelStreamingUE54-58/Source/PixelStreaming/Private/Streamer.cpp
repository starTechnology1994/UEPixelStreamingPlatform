...
namespace UE::PixelStreaming
{
	...
	void FStreamer::StartStreaming()
	{
		...
		{
			UE_LOG(LogPixelStreaming, Log, TEXT("Attempted to start streamer (%ls) but no signalling server URL has been set. Use Streamer->SetSignallingServerURL(URL) or -PixelStreamingURL="), *StreamerId);
			return;
		}
	...
	void FStreamer::SendPlayerMessage(FPixelStreamingPlayerId PlayerId, uint8 Type, const FString& Descriptor)
	{
	...
	void FStreamer::SendFileData(const TArray64<uint8>& ByteData, FString& MimeType, FString& FileExtension)
	{
		...
		Players->Apply([&ByteData, &MimeType, &FileExtension, this](FPixelStreamingPlayerId PlayerId, FPlayerContext& PlayerContext) {
			if (PlayerContext.DataChannel)
			{
				...
				{
					UE_LOG(LogPixelStreaming, Error, TEXT("Unable to send file data over the data channel for player %ls."), *PlayerId);
				}
			}
		});
	}
	...
	void FStreamer::PlayerRequestsBitrate(FPixelStreamingPlayerId PlayerId, int MinBitrate, int MaxBitrate)
	{
		...
		{
			UE_LOG(LogPixelStreaming, Warning, TEXT("Unable to set bitrates from player %ls since they are not the Quality Controller."), *PlayerId);
		}
	}
	...
	void FStreamer::OnAnswer(FPixelStreamingPlayerId PlayerId, const FString& Sdp)
	{
		...
		{
			UE_LOG(LogPixelStreaming, Error, TEXT("Failed to find player id %ls for incoming answer."), *PlayerId);
		}
	}
	...
	void FStreamer::SendInitialSettings(FPixelStreamingPlayerId PlayerId) const
	{
		...
		if (const FPlayerContext* PlayerContext = Players->Find(PlayerId))
		{
			if (PlayerContext->DataChannel)
			{
				...
				{
					UE_LOG(LogPixelStreaming, Log, TEXT("Failed to send initial Pixel Streaming settings to player %ls."), *PlayerId);
				}
			}
		}
	}
		...
		for (EPixelStreamingMessageDirection MessageDirection : PixelStreamingMessageDirections)
		{
			...
			if (const FPlayerContext* PlayerContext = Players->Find(PlayerId))
			{
				if (PlayerContext->DataChannel)
				{
					...
					{
						UE_LOG(LogPixelStreaming, Warning, TEXT("Failed to send Pixel Streaming protocol to player %ls. This player will use the default protocol specified in the front end"), *PlayerId);
					}
				}
			}
		}
	...
	void FStreamer::HandleSetQualityControllerRequest(const FPixelStreamingPlayerId& PlayerId)
	{
		...
		UE_LOG(LogPixelStreaming, Verbose, TEXT("Player %ls has requested quality control through the data channel."), *PlayerId);
		...
		case Settings::GrantOnlyFirst:
			...
			{
				UE_LOG(LogPixelStreaming, Verbose, TEXT("Dropped quality control request from player %ls."), *PlayerId);
			}
			break;
		}
	}
